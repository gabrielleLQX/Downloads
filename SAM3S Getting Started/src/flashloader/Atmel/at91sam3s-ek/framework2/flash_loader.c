...
{
#if USE_ARGC_ARGV
  theFlashParams.count = FlashInit(theFlashParams.base_ptr,
...
{
  theFlashParams.count = FlashWrite(theFlashParams.base_ptr,
...
void Fl2FlashEraseWriteEntry()
{
  ...
  {
    tmp = FlashErase(theFlashParams.base_ptr,
                     theFlashParams.block_size);
    if (tmp == 0)
    {
      tmp = FlashWrite(theFlashParams.base_ptr,
                       theFlashParams.offset_into_block,
                       theFlashParams.count,
                       theFlashParams.buffer);
    }
...
{
  theFlashParams.count = FlashChecksum(theFlashParams.base_ptr,
                                       theFlashParams.count);
}
